  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.85)" }}>
      <div className="glass-panel w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-fade-in">
        <div className="sticky top-0 flex items-center justify-between px-4 py-3 border-b border-cyan-900/40" style={{ background: "#020d1f" }}>
          <div>
            <div className="font-mono text-xs text-cyan-600">INCIDENT DETAIL</div>
            <div className="font-display font-bold text-lg text-white">{incident.incident_code}</div>
          </div>
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {[
              ...
              <div key={k} className="glass-panel-dark p-2.5">
                <div className="font-mono text-[10px] text-slate-600 mb-1">{k}</div>
            ...
            <div className="space-y-3">
              {incident.timeline?.map((ev, i) => (
                <div key={i} className="flex gap-3">
                  ...
                  <div className="pb-2">
                    <div className="font-mono text-xs text-slate-300">{ev.event}</div>
                    <div className="font-mono text-[10px] text-slate-600">{new Date(ev.time).toLocaleTimeString()}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Notes */}
          <div className="glass-panel-dark p-3">
            <div className="font-mono text-xs text-cyan-400 mb-2">OPERATOR NOTES</div>
            <textarea
              defaultValue={incident.notes}
              placeholder="Add operator notes..."
              className="w-full px-2 py-1.5 font-mono text-xs text-slate-300 border border-slate-700 focus:border-cyan-500 outline-none resize-none"
              rows={3}
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <h1 className="font-display font-bold text-2xl text-white tracking-wide">Incident Management</h1>
        </div>
            ...
            <tbody className="divide-y divide-slate-800/40">
              {filtered.map(inc => (
                <tr key={inc.id} className="hover:bg-cyan-900/10 transition-colors">
                  <td className="px-3 py-2 font-mono text-xs text-slate-300">{inc.incident_code}</td>
                  <td className="px-3 py-2 font-mono text-xs text-slate-500 whitespace-nowrap">{new Date(inc.timestamp).toLocaleString()}</td>
                  <td className="px-3 py-2 font-mono text-xs text-slate-400">{inc.camera_id}</td>
                  <td className="px-3 py-2 font-mono text-xs text-slate-500">{inc.sector}</td>
                  <td className="px-3 py-2 font-mono text-xs text-slate-400">{inc.tracking_id}</td>
                  <td className="px-3 py-2 font-mono text-xs text-slate-300">{inc.threat_type}</td>
                  <td className="px-3 py-2 font-mono text-xs text-slate-400">{inc.confidence}%</td>
