...
const TOOLTIP_STYLE = { background: "#0a0f1e", border: "1px solid rgba(34,211,238,0.2)", borderRadius: 0, fontFamily: "JetBrains Mono, monospace", fontSize: 11, color: "#94a3b8" };
  ...
  return (
    <div className="p-4 space-y-4">
      <div>
        ...
        <h1 className="font-display font-bold text-2xl text-white tracking-wide">Threat Intelligence & Analytics</h1>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <div className="glass-panel p-4">
          <div className="font-mono text-xs text-cyan-400 mb-4">THREAT ACTIVITY TIMELINE — 24H</div>
        ...
        {/* Threats by camera */}
        <div className="glass-panel p-4">
          <div className="font-mono text-xs text-cyan-400 mb-4">THREATS BY CAMERA</div>
          <ResponsiveContainer width="100%" height={200}>
        ...
        {/* Risk distribution */}
        <div className="glass-panel p-4">
          <div className="font-mono text-xs text-cyan-400 mb-4">RISK DISTRIBUTION</div>
            ...
            <div className="space-y-2">
              {riskData.map(r => (
                <div key={r.name} className="flex items-center gap-2">
                  ...
                  <div className="font-mono text-xs text-slate-400">{r.name}</div>
        ...
        {/* Sector threat table */}
        <div className="glass-panel p-4">
          <div className="font-mono text-xs text-cyan-400 mb-4">SECTOR THREAT SUMMARY</div>
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-800/30">
              {[
                ...
                <tr key={row.sector} className="hover:bg-cyan-900/10 transition-colors">
                  <td className="py-2 font-mono text-xs text-cyan-300">{row.sector}</td>
                  <td className="py-2 font-mono text-xs text-slate-400">{row.cameras}</td>
                  <td className="py-2 font-mono text-xs text-amber-400">{row.alerts}</td>
                  <td className="py-2 font-mono text-xs text-slate-400">{row.incidents}</td>
                  <td className="py-2">
