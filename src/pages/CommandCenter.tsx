    ...
    <div className="p-4 space-y-4 min-h-full grid-bg">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="font-display font-bold text-2xl text-white tracking-wide">Command Center</h1>
        </div>
      ...
      {demoRunning && (
        <div className="glass-panel p-3 space-y-2">
          ...
          <div className="h-1 bg-slate-800 overflow-hidden">
            <div className="h-full bg-cyan-400 transition-all duration-700" style={{ width: `${(Math.min(demoStep, DEMO_STEPS.length) / DEMO_STEPS.length) * 100}%` }} />
          </div>
          {demoStep > 0 && demoStep <= DEMO_STEPS.length && (
            <div className="font-mono text-xs text-cyan-300">{DEMO_STEPS[Math.min(demoStep, DEMO_STEPS.length) - 1]?.description}</div>
          )}
        </div>
      )}
            ...
            <div className="divide-y divide-slate-800/50">
              {tracks.map(t => (
                <div key={t.tracking_id} className="flex items-center gap-3 px-3 py-2 hover:bg-cyan-900/10 transition-colors">
                  <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: t.status === "ACTIVE" ? "#22c55e" : "#64748b" }} />
                  <div className="font-mono text-xs text-slate-300 w-16 flex-shrink-0">{t.tracking_id}</div>
            ...
            <div className="max-h-72 overflow-y-auto divide-y divide-slate-800/40">
              {alerts.map(alert => (
                <div key={alert.id} className={`p-2.5 transition-colors ${!alert.acknowledged && alert.severity === "CRITICAL" ? "border-glow-red" : ""}`}
                  ...
                  <div className="font-mono text-xs text-slate-200 mt-0.5 leading-tight">{alert.title}</div>
            ...
            <div className="divide-y divide-slate-800/40">
              {incidents.slice(0, 4).map(inc => (
                <div key={inc.id} className="px-3 py-2 hover:bg-cyan-900/10 transition-colors cursor-pointer" onClick={() => nav("/incidents")}>
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs text-slate-300">{inc.incident_code}</span>
                  ...
                  <div className="font-mono text-[10px] text-slate-500 mt-0.5">{inc.threat_type}</div>
                  <div className="font-mono text-[10px] text-slate-600 mt-0.5">{inc.camera_id} | {new Date(inc.timestamp).toLocaleTimeString()}</div>
                </div>
              ))}
            </div>
          </div>
