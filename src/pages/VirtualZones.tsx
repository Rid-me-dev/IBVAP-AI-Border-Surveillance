    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <h1 className="font-display font-bold text-2xl text-white tracking-wide">Virtual Zones / Geofence</h1>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="xl:col-span-2 glass-panel overflow-hidden">
          <div className="flex items-center gap-2 px-3 py-2 border-b border-cyan-900/30 flex-wrap">
            <span className="font-mono text-xs text-cyan-400">ZONE OVERLAY —</span>
          ...
          <div className="relative" style={{ paddingBottom: "56.25%", background: "#000" }}>
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
              ...
              <rect x="0" y="0" width="100" height="100" fill="#050d1e" />
              {/* Grid */}
        ...
        <div className="space-y-3">
          <div className="glass-panel divide-y divide-slate-800/40">
            <div className="px-3 py-2">
              <span className="font-mono text-xs text-cyan-400">ACTIVE ZONES</span>
            </div>
            {cameraZones.map(zone => (
              <div key={zone.id} className="px-3 py-3">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs text-slate-200">{zone.name}</span>
                  </div>
                  <button onClick={() => toggleZone(zone.id)} className={`font-mono text-[10px] px-1.5 py-0.5 border transition-colors ${zone.active ? "border-green-800 text-green-400" : "border-slate-700 text-slate-600"}`}>
                    {zone.active ? "ACTIVE" : "INACTIVE"}
                  </button>
                </div>
                <div className="grid grid-cols-2 gap-1 text-[10px] font-mono">
                  <div className="text-slate-600">Type</div><div style={{ color: ZONE_COLORS[zone.type] }}>{zone.type}</div>
                  <div className="text-slate-600">Camera</div><div className="text-slate-400">{zone.camera_id}</div>
                  <div className="text-slate-600">Sector</div><div className="text-slate-400">{zone.sector}</div>
                  <div className="text-slate-600">Risk Weight</div><div className="text-slate-400">{zone.risk_weight}/10</div>
                </div>
                <div className="mt-2 pt-2 border-t border-slate-800">
                  <div className="font-mono text-[10px] text-slate-600 mb-1">ZONE RULE</div>
          ...
          {showAddZone && (
            <div className="glass-panel p-3 animate-fade-in">
              ...
              <div className="space-y-2">
                <div>
                  <label className="block font-mono text-[10px] text-cyan-700 mb-1">ZONE NAME</label>
                  <input value={newZoneName} onChange={e => setNewZoneName(e.target.value)} className="w-full px-2 py-1.5 font-mono text-xs text-slate-200 border border-slate-700 focus:border-cyan-500 outline-none" style={{ background: "rgba(15,23,42,0.8)" }} />
                </div>
                <div>
                ...
                <div className="flex gap-2">
                  <button onClick={addZone} className="flex-1 py-1.5 font-mono text-xs" style={{ background: "#22d3ee", color: "#020817" }}>ADD</button>
                  <button onClick={() => setShowAddZone(false)} className="px-3 py-1.5 font-mono text-xs border border-slate-700 text-slate-400">CANCEL</button>
                </div>
              </div>
            </div>
          )}
