    ...
    <div className="p-4 space-y-4 min-h-full">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <h1 className="font-display font-bold text-2xl text-white tracking-wide">Live Surveillance</h1>
        </div>
          ...
          {filtered.map(cam => (
            <div key={cam.id} className={`glass-panel overflow-hidden cursor-pointer transition-all ${selected && selected.id === cam.id ? "border-glow-cyan" : ""}`}
              ...
              <div className="px-3 py-2 flex items-center justify-between border-t border-cyan-900/20">
                <div>
                  <div className="font-mono text-xs text-slate-300">{cam.camera_id} — {cam.name}</div>
                  <div className="font-mono text-[10px] text-slate-500">Sector {cam.sector} | {cam.type}</div>
                </div>
                <div className="text-right">
                  <div className="font-mono text-[10px]" style={{ color: cam.status === "ONLINE" ? "#22c55e" : "#ef4444" }}>{cam.status}</div>
                  <div className="font-mono text-[10px] text-slate-600">AI: {cam.ai_status}</div>
                </div>
              </div>
            </div>
          ))}
          ...
          <div className="space-y-2">
            ...
            {filtered.map(cam => (
              <button key={cam.id} onClick={() => setSelected(cam)}
                className={`w-full text-left px-2 py-1.5 font-mono text-xs border transition-colors ${selected.id === cam.id ? "border-cyan-600 text-cyan-400" : "border-slate-800 text-slate-500 hover:border-slate-600 hover:text-slate-300"}`}
                style={{ background: selected.id === cam.id ? "rgba(34,211,238,0.06)" : "rgba(15,23,42,0.6)" }}>
                <div className="flex items-center justify-between">
                  <span>{cam.camera_id} — {cam.name}</span>
                  <span style={{ color: cam.status === "ONLINE" ? "#22c55e" : "#ef4444" }}>●</span>
                </div>
              </button>
            ))}
          </div>
            ...
            <form className="space-y-3" onSubmit={e => { e.preventDefault(); setShowAddModal(false); }}>
              {[
                ...
                <div key={label}>
                  ...
                  <input type={type} placeholder={placeholder} className="w-full px-3 py-2 font-mono text-sm text-slate-200 border border-slate-700 focus:border-cyan-500 outline-none" style={{ background: "rgba(15,23,42,0.8)" }} />
                </div>
              ))}
              <div className="flex gap-2 pt-2">
                <button type="submit" className="flex-1 py-2 font-mono text-sm tracking-widest" style={{ background: "#22d3ee", color: "#020817" }}>ADD CAMERA</button>
                <button type="button" onClick={() => setShowAddModal(false)} className="px-4 py-2 font-mono text-sm border border-slate-700 text-slate-400 hover:border-slate-500">CANCEL</button>
              </div>
            </form>
          </div>
        </div>
